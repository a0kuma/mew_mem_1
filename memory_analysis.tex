...
Therefore, the persistent AdamW state cost is exactly $2 \times$ the parameter size.
...
the weight, its gradient, \texttt{exp\_avg}, and \texttt{exp\_avg\_sq}. This matches the
$3 \times$ extra-over-params expectation when gradients remain live, with no additional
persistent buffer attributable to a stored square root.
...
\texttt{memory\_snapshot\_all\_final.pickle} contains Adam-related allocation events but no distinct,
