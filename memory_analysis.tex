...
%=============================================================================
\section{AdamW sqrt and the 2x vs 3x multiplier}
\label{sec:adam_sqrt}
%=============================================================================

Both \texttt{gpt2\_gpipe\_memory\_profile.py} and \texttt{why\_adam\_times\_3\_here/iiiandadam.py} construct
\texttt{torch.optim.AdamW}. AdamW maintains two persistent state tensors per parameter: \texttt{exp\_avg}
and \texttt{exp\_avg\_sq}. The elementwise square root used in the update
\begin{equation}
  \theta \leftarrow \theta - \eta \cdot \frac{m_t}{\sqrt{v_t} + \epsilon}
\end{equation}
is computed on the fly during \texttt{optimizer.step()} and is not stored as an additional state tensor.
Therefore, the persistent AdamW state cost is exactly $2 \times$ the parameter size. \texttt{torch._foreach_sqrt}
does create a temporary tensor during the step; if a snapshot is taken immediately after
\texttt{optimizer.step()}, that temporary allocation can still appear as live due to asynchronous
kernel execution or allocator caching. This is a transient buffer, not a persistent optimizer state.

Let $|\theta|$ be the number of parameter elements and $\beta$ the bytes per element. The memory terms are
\begin{align}
  M_{\text{params}} &= |\theta| \, \beta \\
  M_{\text{state}}  &= 2 |\theta| \, \beta \\
  M_{\text{grads}}  &= |\theta| \, \beta \quad \text{(when gradients are live)}
\end{align}
so:
\begin{align}
  M_{\text{params}} + M_{\text{state}} &= 3 |\theta| \, \beta \quad \text{(2x extra over params)} \\
  M_{\text{params}} + M_{\text{state}} + M_{\text{grads}} &= 4 |\theta| \, \beta \quad \text{(3x extra over params)}
\end{align}

\noindent\textbf{Adam demo snapshot.}
In \texttt{iiiandadam.py}, \texttt{nn.Linear(10000, 1000)} has $10{,}000{,}000$ weights
($40{,}000{,}000$ bytes) plus a 1,000 element bias. The snapshot
\texttt{gpu\_memory\_snapshot-adam.pickle} is taken after \texttt{optimizer.step()} without clearing gradients.
Its active blocks include four allocations of size $40{,}000{,}000$ bytes, matching
the weight, its gradient, \texttt{exp\_avg}, and \texttt{exp\_avg\_sq}. The trace also shows
\texttt{\_foreach\_sqrt} in the call stack, indicating a temporary buffer allocation for the
step, but there is no additional long-lived $40{,}000{,}000$-byte block beyond those four.
This matches the $3 \times$ extra-over-params expectation when gradients remain live.

\noindent\textbf{GPT-2 GPipe snapshot.}
In \texttt{gpt2\_gpipe\_memory\_profile.py}, the optimizer is also AdamW. The peak-memory analysis in
Sections~\ref{sec:optimizer} and~\ref{sec:gradients} shows optimizer state tensors at exactly
$2 \times$ the parameter total and gradients at $1 \times$ the parameter total on GPU\,0, which is
the same $3 \times$ extra-over-params pattern. The device trace in
\texttt{memory\_snapshot\_all\_final.pickle} contains Adam-related allocation events, which can
include transient \texttt{\_foreach\_sqrt} buffers depending on snapshot timing, but no distinct,
long-lived buffer corresponding to a stored square root.

\noindent\textbf{Peak vs. end-of-run snapshot.}
The file \texttt{memory\_snapshot\_all\_final.pickle} is a single snapshot taken at the end of the
training loop, so it can reflect a state after \texttt{optimizer.step()} and \texttt{zero\_grad()} where
gradients are no longer live. In that case, only parameters and AdamW state remain (about $2\times$
extra over params). The true peak for GPT-2 is determined by replaying the full allocation history
(Section~\ref{sec:peak_method}); it occurs earlier during backward recomputation when gradients and
checkpoint activations overlap, which is why the peak total exceeds the end-of-run snapshot.
